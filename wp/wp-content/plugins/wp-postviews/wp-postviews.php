...
function the_viewer() {
    $ret = '';
    $viewed = get_post_meta( get_the_ID(), 'whoviewed');
    if($viewed){
        $ret = $viewed[0];
    }
    return $ret;
...
//$month 接受 01到12
function the_viewinmonth($id, $month) {
    $ret = 0;
    $viewed = get_post_meta( $id, 'viewsinmonth'.$month );
    if($viewed){
        $ret = intval($viewed[0]);
    }
    return $ret;
...
function whoviewed($post_id){
    global $current_user;
    $whoviewed = get_post_meta( $post_id, 'whoviewed' );

    if(!$whoviewed){
        add_post_meta($post_id, 'whoviewed', $current_user->display_name);
    }else{
        if(false == strstr($whoviewed[0], $current_user->display_name)){
            update_post_meta( $post_id, 'whoviewed',$whoviewed[0]." ,".$current_user->display_name);
        }
    }
...
function viewsinmonth($post_id){
    $whenviewed = get_post_meta( $post_id, 'viewsinmonth'.date('m', time()) );

    if(!$whenviewed){
        add_post_meta($post_id, 'viewsinmonth'.date('m', time()), 1, true);
    }else {
        $whenviewed = intval($whenviewed[0]);
        update_post_meta( $post_id, 'viewsinmonth'.date('m', time()),$whenviewed + 1 );
    }
}
