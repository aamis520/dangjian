...
<form id="site_post_form" class="djd_site_post_form bordered" method="post" action="<?php echo admin_url('admin-ajax.php'); ?>" enctype="multipart/form-data">
	...
	<div id="field-wrapper">
		<legend><?php echo ( $djd_options['djd-form-name'] ? $djd_options['djd-form-name'] : "发表文章"); ?></legend>
...
<!--					--><?php //} ?>
		...
		if ( ($djd_options['djd-guest-info']) && (!is_user_logged_in()) ){ ?>
			...
			<input type="text" required id="djd_site_post_guest_name" name="djd_site_post_guest_name" maxlength="40" />

			<label for="djd_site_post_guest_email"><?php _e('Your Email', 'djd-site-post'); ?></label>
			<input type="email" required id="djd_site_post_guest_email" name="djd_site_post_guest_email" maxlength="40" /><br><br>
		<?php } ?>
	...
	<?php if ( ($djd_options['djd-quiz']) && (!is_user_logged_in()) ) { ?>
		...
		<label for="djd_quiz" id="djd_quiz_label"><?php echo $no1; ?> 加 <?php echo $no2; ?> 等于</label>
		<input type="text" required id="djd_quiz" name="djd_quiz" maxlength="2" size="2" />
