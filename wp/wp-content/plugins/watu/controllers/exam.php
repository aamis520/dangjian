...
add_action("watu_exams", "watu_exams");
function watu_exams() {
	global $wpdb;
...
function watu_exam() {
	...
	if(isset($_POST['submit']) and check_admin_referer('watu_create_edit_quiz')) {
        ...
        $trainingvideo = sanitize_text_field($_POST['video']);
        $randomize = empty($_POST['randomize']) ? 0 : 1;
		$single_page = 1 ;//永远是单页的
		...
		$take_again = 1;//empty($_POST['take_again']) ? 0 : 1;
		...
		if($_REQUEST['action'] == 'edit') { //Update goes here
			...
			$wpdb->query($wpdb->prepare("UPDATE ".WATU_EXAMS."
				SET name=%s, trainingvideo=%s, description=%s,final_screen=%s, randomize=%d, single_page=%d, 
				...
				WHERE ID=%d", $name, $trainingvideo, $description, $content,
			...
			$wpdb->query($wpdb->prepare("INSERT INTO ".WATU_EXAMS." 
				(name, trainingvideo, description, final_screen,  added_on, randomize, single_page, show_answers, require_login, 
				...
				advanced_settings) 
				VALUES(%s, %s,%s, %s, NOW(), %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %s, %d, %s, %d, %d, %d, %d, %d, %s)",
				$name, $trainingvideo, $description, $content, $randomize, $single_page,
