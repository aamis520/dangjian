...
<form action="" method="post" class="form" role="form" id="quiz-<?php echo $exam_id?>" <?php if(!empty($exam->no_ajax)):?>onsubmit="return Watu.submitResult(this)"<?php endif;?>>
<?php
...
foreach ($questions as $qct => $ques) {
	...
	$output .= "<div class='watu-question form-group' id='question-$question_count'>";
	$output .= "<div class='question-content form-group'>". wpautop($question_number .  stripslashes($ques->question)) ;
	...
	foreach ($dans as $ans) {
		...
		$output .= wpautop(" <label><input type='$ans_type' name='answer-{$ques->ID}[]' id='answer-id-{$ans->ID}' class='answer answer-$question_count $answer_class answerof-{$ques->ID}' value='{$ans->ID}' /><span>  ".stripslashes($ans->answer)."</span></label>");
	}
...
<?php if(empty($exam->no_ajax)):?>
    <center>
    <input type="button" class="btn btn-danger" name="action" onclick="Watu.submitResult()" id="action-button" value="提交答案"  />
    </center>
<?php else:?>
