<?php if (!$in_shortcode): ?>
<div class="wrap">
    <h2><?php printf(__("参加培训情况： '%s'", 'watu'), $exam->name); ?></h2>

    <p><a href="admin.php?page=watu_exams"><?php _e('回到问题列表', 'watu') ?></a>
        <?php if ($count): ?>&nbsp;|&nbsp;
            <a href="#"
               onclick="jQuery('#filterForm').toggle('slow');return false;"><?php _e('Filter/search these records', 'watu') ?></a>
            &nbsp;|&nbsp;
    ...
    <div class="wrap" style="min-width:60%;margin-right:2%;float:left">
        <div id="filterForm"
             style="display:<?php echo $display_filters ? 'block' : 'none'; ?>;margin-bottom:10px;padding:5px;"
             class="widefat">
            <form method="get" class="watu-admin" action="admin.php">
                ...
                <div><label><?php _e('Date Taken', 'watu') ?></label> <select name="datef">
                    ...
                    </select> <input type="text" name="date" value="<?php echo @$_GET['date'] ?>"> <i>YYYY-MM-DD</i>
                </div>
                ...
                <div><label><?php _e('Grade/result equals:', 'watu') ?></label> <select name="grade_id">
                        ...
                        <?php endforeach; ?>
                    </select> <?php _e('(This filter will exclude grades earned prior to Watu version 2.4.7)', 'watu') ?>
                </div>
                ...
                <div><input type="submit" value="<?php _e('Search/Filter', 'watu') ?>">
                    <input type="button" value="<?php _e('Clear Filters', 'watu') ?>"
                           onclick="window.location='admin.php?page=watu_takings&exam_id=<?php echo $exam->ID; ?>';">
                </div>
            </form>
        </div>
        ...
        if ($count):?>
            ...
            <table class="<?php echo $in_shortcode ? 'watu-table' : 'widefat' ?> wp-list-table">
                <tr>
                    <th>
                        <a href="<?php echo $target_url ?>&ob=tU.user_login&offset=<?php echo $offset ?>&dir=<?php echo $odir ?>&<?php echo $filters_url; ?>"><?php _e('Name & Email', 'watu'); ?></a>
                    </th>
                    <?php if (!$in_shortcode): ?>
                        <th>
                        <a href="<?php echo $target_url ?>&ob=tT.ip&offset=<?php echo $offset ?>&dir=<?php echo $odir ?>&<?php echo $filters_url; ?>"><?php _e('IP', 'watu'); ?></a>
                        </th><?php endif; ?>
                    <th>
                        <a href="<?php echo $target_url ?>&ob=tT.date&offset=<?php echo $offset ?>&dir=<?php echo $odir ?>&<?php echo $filters_url; ?>"><?php _e('Date', 'watu') ?></a>
                    </th>
                    <th>
                        <a href="<?php echo $target_url ?>&ob=tT.points&offset=<?php echo $offset ?>&dir=<?php echo $odir ?>&<?php echo $filters_url; ?>"><?php _e('Points', 'watu') ?></a>
                    </th>
                    <th><?php _e('Result', 'watu') ?></th>
                    <?php if (!$in_shortcode): ?>
                        <th><?php _e('Details', 'watu') ?></th>
                        <th><?php _e('Delete', 'watu') ?></th>
                    <?php endif; // end if not in shortcode;
                    ?></tr>
                ...
                <?php foreach ($takings as $taking):
                    ...
                    <tr class="<?php echo $class ?>">
                        ...
                        <?php if (!$in_shortcode): ?>
                            <td><?php echo $taking->ip; ?></td><?php endif; ?>
                        ...
                        <?php if (!$in_shortcode): ?>
                            <td><?php if (empty($taking->snapshot)): _e('n/a', 'watu');
                                          ...
                                          onclick="Watu.takingDetails('<?php echo $taking->ID ?>');return false;"><?php _e('view', 'watupro') ?></a><?php endif; ?>
                            </td>
                            <td><a href="#"
                                   onclick="WatuDelTaking('<?php echo wp_nonce_url('admin.php?page=watu_takings', 'watu_del_taking'); ?>&exam_id=<?php echo $exam->ID ?>&del_taking=1&id=<?php echo $taking->ID ?>');return false;"><?php _e('Delete', 'watu') ?></a>
                            </td>
                        <?php endif; // end if not in shortcode
                        ?>
                    </tr>
            ...
            <p align="center"><?php if ($offset > 0): ?><a
                    href="<?php echo $target_url ?>&offset=<?php echo($offset - 10) ?>&ob=<?php echo $ob ?>&dir=<?php echo $dir ?>&<?php echo $filters_url; ?>"><?php _e('Previous page', 'watu') ?></a><?php endif; ?>

                <?php if ($offset + 10 < $count): ?> <a
                        href="<?php echo $target_url ?>&offset=<?php echo($offset + 10) ?>&ob=<?php echo $ob ?>&dir=<?php echo $dir ?>&<?php echo $filters_url; ?>"><?php _e('Next page', 'watu') ?></a> <?php endif; ?>
            </p>
