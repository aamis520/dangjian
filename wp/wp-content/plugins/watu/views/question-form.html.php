<div class="wrap">
    ...
    <p><a href="admin.php?page=watu_questions&quiz=<?php echo $_GET['quiz'] ?>"><?php _e('回到问题列表', 'watu') ?></a>
    ...
    <style type="text/css">
        .qtrans_title, .qtrans_title_wrap {
            display: none;
        }
    </style>
    ...
    <form name="post"
          action="admin.php?page=watu_questions&amp;quiz=<?php echo $_GET['quiz']; ?>&amp;action=<?php echo empty($question->ID) ? 'new' : 'edit' ?>"
          method="post" id="post">
        <div id="poststuff">
            ...
            <div class="postarea">
                ...
                <div class="postbox">
                    <h3 class="hndle"><?php _e('问题描述', 'watu') ?></span></h3>
                    <div class="inside">
                ...
                <div class="postbox" id="atdiv">
                    <h3 class="hndle"><span><?php _e('答案选择类型', 'watu') ?></span></h3>
                    <div class="inside" style="padding:8px">
                        ...
                        ?>
                        <label>&nbsp;<input type='radio' name='answer_type' <?php print $single ?> id="answer_type_r"
                                            value='radio'
                                            onclick="jQuery('#watuOpenEndAnswers').hide();"/> <?php _e('单选', 'watu') ?>
                        </label>
                        &nbsp;&nbsp;&nbsp;
                        <label>&nbsp;<input type='radio' name='answer_type' <?php print $multi ?> id="answer_type_c"
                                            value='checkbox'
                                            onclick="jQuery('#watuOpenEndAnswers').hide();"/> <?php _e('多选', 'watu') ?>
                        </label>
                ...
                <div class="postbox" id="questionAnswers">
                    <h3 class="hndle"><span><?php _e('答案设置', 'watu') ?></span></h3>
                    <div class="inside">
                        <p id="watuOpenEndAnswers"
                           style="display:<?php echo ($ans_type == 'textarea') ? 'block' : 'none'; ?>"><?php printf(__('Answers to open-end questions will be considered matched when there is exact case-insensitive match. For more flexibility check <a href="%s" target="_blank">WatuPRO</a>', 'watu'), 'http://calendarscripts.info/watupro'); ?></p>
                        <?php
                        for ($i = 1; $i <= $answer_count; $i++) { ?>
                            <p style="border-bottom:1px dotted #ccc"><textarea name="answer[]" class="answer" rows="3"
                                                                               cols="50"><?php if ($action == 'edit') echo stripslashes(@$all_answers[$i - 1]->answer); ?></textarea>
                                <label for="correct_answer_<?php echo $i ?>"><?php _e("是否为正确答案？", 'watu'); ?></label>
                                <input type="<?php print ($ans_type == 'radio') ? 'radio' : 'checkbox' ?>"
                                       class="correct_answer"
                                       id="correct_answer_<?php echo $i ?>" <?php if (@$all_answers[$i - 1]->correct == 1) echo 'checked="checked"'; ?>
                                       name="correct_answer[]" value="<?php echo $i ?>"/>
                                <label style="margin-left:10px"><?php _e('答对后得分:', 'watu') ?> <input type="text"
                                                                                                      class="numeric"
                                                                                                      size="4"
                                                                                                      name="point[]"
                                                                                                      value="<?php if ($action == 'edit') echo stripslashes(@$all_answers[$i - 1]->point); ?>"></label>
                            </p>
                        <?php } ?>
                        <style>#extra-answers p {
                                border-bottom: 1px dotted #ccc;
                            }</style>
                        <div id="extra-answers"></div>
                        <a href="javascript:newAnswer();"><?php _e("增加新答案", 'watu'); ?></a>
