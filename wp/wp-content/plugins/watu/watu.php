...
include( WATU_PATH.'/views/unitythemelist.php');
...
function watu_init() {
	...
	if($version != '2.67') watu_activate(true);
...
function watu_add_menu_links() {
	...
	add_menu_page(__('党建视频培训', 'watu'), __('培训列表', 'watu'), $view_level, 'watu_exams', 'watu_exams');
	add_submenu_page('watu_exams', __('设置', 'watu'), __('设置', 'watu'), $view_level , 'watu_options', 'watu_options');
...
add_action( 'watu_showdetail', 'watu_action_showdetail' );

function watu_action_showdetail( $id ) {
    $exam_id = $id;
...
function watu_activate($update = false) {
	...
	if($wpdb->get_var("SHOW TABLES LIKE '".WATU_EXAMS."'") != WATU_EXAMS) {
		$sql = "CREATE TABLE `".WATU_EXAMS."`(
					...
					trainingvideo varchar(254) NOT NULL DEFAULT '',
					description mediumtext ,
					final_screen mediumtext ,
	...
	update_option( "watu_version", '2.67' );
