...
<script type="text/javascript">
    $(function () {
        ...
        $("#topics").on('click','a.zannum',function () {
            ...
            var $self = $(this);
            $.ajax({
                url:"<?php echo getapiurl('goldentimesupdatezantopicapi')?>",
                dataType:"json",
                cache:false,
                type:"get",
                data:{
        ...
        $("#sendtopicbtn").click(function(){
            ...
            //先提交内容
            $.ajax({
                url:"<?php echo getapiurl('goldentimescreatetopicapi')?>",
                dataType:'json',
                cache:false,
                type:"get",
                data:{
        ...
        $("#topics").on('click','button.sendreply',function () {
            ...
            //更新replynum
            $.ajax({
                url:"<?php echo getapiurl('goldentimesupdatereplynumtopicapi')?>",
                dataType:"json",
                type:"get",
                cache:false,
                data:{
            ...
            //创建reply
            $.ajax({
                url:"<?php echo getapiurl('goldentimescreatereplyapi')?>",
                dataType:"json",
                type:"get",
                cache:false,
                data:{
                    ...
                    if(res && res.id) {
                        ...
                        if("<?php echo $_SESSION['userid']?>" != topicownerid) {
                            ...
                            $.ajax({
                                ...
                                error: function () {notifybox("error", "网络异常，请稍后重试")
                                }
                            });
                        }
    ...
    function init(topicskip,topiclimit,replyskip,replylimit) {
        var ownerid = "<?php echo $_SESSION['userid']?>";
        $.ajax({
            url:"<?php echo getapiurl('goldentimeslisttopicapi')?>",
            dataType:"json",
            cache:false,
            type:"get",
            data:{
                    ...
                    $.each(res.topics,function (index,topic) {
                        ...
                        if(topic.reply && topic.reply.length > 0){
                            ...
                            if(topic.reply && topic.replynum > replylimit) {
                                ...
                                $html += '<a href="?page=goldenTimesMoreReplys&topicid='+topic.id+'" class="btn btn-xs btn-single btn-info pull-right">查看更多评论</a>';
                                $html += '</div>';
                            }
                        }
                        $html += '<form class="story-comment-form">';
