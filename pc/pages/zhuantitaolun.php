...
<!--                                                </button>-->
            ...
            <section id="nothas" class="text-center text-muted" style="margin-top:10px;font-size:14px;background: #fff;padding:10px 0;display: none;">暂无最新消息
        </div>
    </div>
</section>
...
<script type="text/javascript">
    $(function () {
        ...
        $("#topics").on('click','a.zannum',function () {
            ...
            var $self = $(this);
            $.ajax({
                url:"<?php echo getapiurl('zhuantidiscussupdatezantopicapi')?>",
                dataType:"json",
                cache:false,
                type:"get",
                data:{
        ...
        $("#sendtopicbtn").click(function(){
            ...
            //先提交内容
            $.ajax({
                url:"<?php echo getapiurl('zhuantidiscusscreatetopicapi')?>",
                dataType:'json',
                cache:false,
                type:"get",
                data:{
        ...
        $("#topics").on('click','button.sendreply',function () {
            ...
            //更新replynum
            $.ajax({
                url:"<?php echo getapiurl('zhuantidiscussupdatereplynumtopicapi')?>",
                dataType:"json",
                type:"get",
                cache:false,
                data:{
            ...
            //创建reply
            $.ajax({
                url:"<?php echo getapiurl('zhuantidiscusscreatereplyapi')?>",
                dataType:"json",
                type:"get",
                cache:false,
                data:{
    ...
    function init(topicskip,topiclimit,replyskip,replylimit) {
        var ownerid = "<?php echo $_SESSION['userid']?>";
        $.ajax({
            url:"<?php echo getapiurl('zhuantidiscusslisttopicapi')?>",
            dataType:"json",
            cache:false,
            type:"get",
            data:{
                    ...
                    $.each(res.topics,function (index,topic) {
                        ...
                        if(topic.reply && topic.reply.length > 0){
                            ...
                            $.each(topic.reply,function (index,reply) {
                                $html += '<li replyid="'+res.id+'" class="eachreply">';
                                $html += '<div class="story-comment">';
                                $html += '<a  class="comment-user-img">';
                            ...
                            if(topic.reply && topic.replynum > replylimit) {
                                ...
                                $html += '<a href="?page=zhuantitaolunMoreReplys&topicid='+topic.id+'" class="btn btn-xs btn-single btn-info pull-right">查看更多评论</a>';
                                $html += '</div>';
                            }
                        }
                        $html += '<form class="story-comment-form">';
